...
# Load admin/teacher credentials from JSON file
ADMIN_FILE = os.path.join(Path(__file__).parent, "admins.json")
with open(ADMIN_FILE, "r") as f:
    admin_data = json.load(f)
TEACHERS = {t["username"]: t["password"] for t in admin_data["teachers"]}
...
def signup_for_activity(activity_name: str, email: str, username: str = Depends(verify_teacher)):
    ...
    return {"message": f"Signed up {email} for {activity_name} by {username}"}
...
def unregister_from_activity(activity_name: str, email: str, username: str = Depends(verify_teacher)):
    ...
    return {"message": f"Unregistered {email} from {activity_name} by {username}"}
